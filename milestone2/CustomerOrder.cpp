    ...
    for (size_t j = 0; j < m_cntItem; j++) {
        ...
        if (m_lstItem[j]->m_itemName == station.getItemName()) {
            ...
            if (station.getQuantity() > 0) {
                ...
                m_lstItem[j]->m_serialNumber = station.getNextSerialNumber();
                m_lstItem[j]->m_isFilled = true;
                station.updateQuantity();
                os << setw(11) << right;
                os << "Filled " << m_name << ", " << m_product << " [" << m_lstItem[j]->m_itemName << "]" << endl;
                break;

            } else {

                os << setw(11) << right;
                os << "Unable to fill " << m_name << ", " << m_product << " [" << m_lstItem[j]->m_itemName << "]" << endl;
                break;
